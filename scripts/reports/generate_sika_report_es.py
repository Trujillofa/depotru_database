...
if s24 and s25:
    ...
    report.append("")
    report.append(
        f"- **Crecimiento de Ingresos:** {rev_growth:+.1f}% interanual (${s24['net_revenue']:,.0f} → ${s25['net_revenue']:,.0f})"
    )
    report.append(
        f"- **Crecimiento de Utilidad:** {profit_growth:+.1f}% interanual (${s24['net_profit']:,.0f} → ${s25['net_profit']:,.0f})"
    )
    report.append(
        f"- **Crecimiento de Clientes:** {cust_growth:+.1f}% interanual ({int(s24['unique_customers']):,} → {int(s25['unique_customers']):,} clientes)"
    )
    report.append(
        f"- **Mejora de Margen:** {(s24['net_profit']/s24['net_revenue']*100):.1f}% → {(s25['net_profit']/s25['net_revenue']*100):.1f}%"
    )
    report.append("")
...
for seg in data["customer_segments"]:
    ...
    report.append(
        f"| {seg_name} | {year} | {int(seg['num_customers']):,} | "
...
if s24 and s25:
    ...
    if rev_growth > 15:
        report.append(
            f"- **Fuerte Crecimiento de Ingresos:** {rev_growth:.1f}% interanual indica demanda robusta del mercado para PRODUCTOS SIKA"
        )
    else:
        report.append(
            f"- **Crecimiento Moderado de Ingresos:** {rev_growth:.1f}% interanual - considerar estrategias de expansión"
        )

    if profit_growth > rev_growth:
        report.append(
            f"- **Expansión de Márgenes:** Utilidad creciendo más rápido que ingresos ({profit_growth:.1f}% vs {rev_growth:.1f}%) muestra mejora en eficiencia operativa"
        )
    ...
    report.append(
        f"- **Crecimiento de Base de Clientes:** {cust_growth:+.1f}% ({int(s24['unique_customers']):,} → {int(s25['unique_customers']):,})"
    )
    ...
    if top_2024:
        ...
        report.append(
            f"- **Concentración de Clientes (2024):** Top 5 clientes representan {top5_pct:.1f}% de ingresos"
        )
    ...
    if top_2025:
        ...
        report.append(
            f"- **Concentración de Clientes (2025):** Top 5 clientes representan {top5_pct_25:.1f}% de ingresos"
        )
        ...
        if top5_pct_25 < top5_pct:
            ...
            report.append(
                "  - ⚠️ Aumento de concentración - considerar iniciativas de adquisición de clientes"
            )
    ...
    if prof_2024 and prof_2025:
        ...
        top_prod_contrib_24 = (
            (top_product_24["revenue"] / s24["net_revenue"] * 100)
            if s24["net_revenue"]
            else 0
        )

        report.append(
            f"- **Producto Estrella (2024):** {top_product_24['product_name'][:50]}"
        )
        report.append(
            f"  - Ingresos: ${top_product_24['revenue']:,.0f} ({top_prod_contrib_24:.1f}% del total)"
        )
        report.append(
            f"- **Producto Estrella (2025):** {top_product_25['product_name'][:50]}"
        )
        report.append(f"  - Ingresos: ${top_product_25['revenue']:,.0f}")

        if top_product_24["product_name"] != top_product_25["product_name"]:
            report.append(
                f"  - 📊 **Cambio en liderazgo:** {top_product_25['product_name'][:40]} superó a {top_product_24['product_name'][:40]}"
            )
    ...
    if cust_growth < 10:
        ...
        report.append(
            "   - Enfocarse en adquisición de nuevos clientes para impulsar crecimiento"
        )
    ...
    returns_growth = (
        ((s25["returns"] - s24["returns"]) / s24["returns"] * 100)
        if s24["returns"]
        else 0
    )
    if returns_growth > 100:
        ...
        report.append(
            f"   - ⚠️ **Incremento crítico de devoluciones:** {returns_growth:+.0f}% ({int(s24['returns']):,} → {int(s25['returns']):,} unidades)"
        )
