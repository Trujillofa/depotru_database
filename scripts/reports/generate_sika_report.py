...
if s24 and s25:
    ...
    report.append("")
    report.append(f"- **Revenue Growth:** {rev_growth:+.1f}% YoY (${s24['net_revenue']:,.0f} → ${s25['net_revenue']:,.0f})")
    report.append(f"- **Profit Growth:** {profit_growth:+.1f}% YoY (${s24['net_profit']:,.0f} → ${s25['net_profit']:,.0f})")
    report.append(f"- **Customer Growth:** {cust_growth:+.1f}% YoY ({int(s24['unique_customers']):,} → {int(s25['unique_customers']):,} customers)")
    report.append(f"- **Margin Improvement:** {(s24['net_profit']/s24['net_revenue']*100):.1f}% → {(s25['net_profit']/s25['net_revenue']*100):.1f}%")
    report.append("")
...
report.append("")
report.append("| Month | 2024 Revenue | 2024 Customers | 2025 Revenue | 2025 Customers | Revenue Growth % |")
report.append("|-------|--------------|----------------|--------------|----------------|------------------|")
...
report.append("")
report.append("| Segment | Year | Customers | Revenue | Avg Revenue/Customer | % of Total Revenue |")
report.append("|---------|------|-----------|---------|---------------------|-------------------|")
...
for seg in data['customer_segments']:
    ...
    report.append(f"| {seg['segment']} | {year} | {int(seg['num_customers']):,} | "
...
report.append("")
report.append("| Subcategory | Year | Products | Customers | Revenue | Profit | Margin % | Transactions |")
report.append("|-------------|------|----------|-----------|---------|--------|----------|--------------|")
...
if s24 and s25:
    ...
    if rev_growth > 15:
        report.append(f"- **Strong Revenue Growth:** {rev_growth:.1f}% YoY indicates robust market demand for PRODUCTOS SIKA")
    else:
        report.append(f"- **Moderate Revenue Growth:** {rev_growth:.1f}% YoY - consider expansion strategies")

    if profit_growth > rev_growth:
        report.append(f"- **Margin Expansion:** Profit growing faster than revenue ({profit_growth:.1f}% vs {rev_growth:.1f}%) shows improving operational efficiency")
    ...
    if monthly_sorted_24:
        ...
        report.append(f"- Best performing month: **{best_month_24}** (${best_rev_24:,.0f})")
    ...
    if monthly_sorted_25 and len(monthly_2025) > 0:
        ...
        report.append(f"- Best performing month: **{best_month_25}** (${best_rev_25:,.0f})")
        report.append("")
    ...
    cust_growth = ((s25['unique_customers'] - s24['unique_customers']) / s24['unique_customers'] * 100)
    report.append(f"- **Customer Base Growth:** {cust_growth:+.1f}% ({int(s24['unique_customers']):,} → {int(s25['unique_customers']):,})")
    ...
    if top_2024:
        ...
        report.append(f"- **Customer Concentration (2024):** Top 5 customers represent {top5_pct:.1f}% of revenue")
    ...
    if top_2025:
        ...
        report.append(f"- **Customer Concentration (2025):** Top 5 customers represent {top5_pct_25:.1f}% of revenue")

        if top5_pct_25 < top5_pct:
            report.append("  - ✅ Reduced concentration indicates healthier customer diversification")
        else:
            report.append("  - ⚠️ Increasing concentration - consider customer acquisition initiatives")
    ...
    if prof_2024:
        ...
        top_prod_contrib = (top_product['revenue'] / s24['net_revenue'] * 100) if s24['net_revenue'] else 0
        report.append(f"- **Star Product (2024):** {top_product['product_name'][:50]} (${top_product['revenue']:,.0f}, {top_prod_contrib:.1f}% of total)")

    if prof_2025:
        top_product_25 = prof_2025[0]
        report.append(f"- **Star Product (2025):** {top_product_25['product_name'][:50]} (${top_product_25['revenue']:,.0f})")
    ...
    if rev_growth > 15 and profit_growth > 20:
        report.append("1. **Scale Operations:** Strong growth trajectory suggests opportunity for capacity expansion")

    if cust_growth < 10:
        report.append("1. **Customer Acquisition:** Focus on new customer acquisition to drive growth")

    if top5_pct > 50:
        report.append("2. **Reduce Concentration Risk:** Develop strategies to diversify customer base")

    report.append("3. **Inventory Optimization:** Align stock levels with monthly demand patterns")
    report.append("4. **Customer Retention:** Implement loyalty programs for high-value customers")
    report.append("5. **Product Mix Analysis:** Focus on high-margin products while maintaining volume leaders")
