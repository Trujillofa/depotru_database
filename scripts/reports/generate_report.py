...
for y in data["yearly_totals"]:
    ...
    report.append(
        f"| {int(y['year'])} | ${y['net_revenue']:,.0f} | ${y['net_profit']:,.0f} | {margin:.1f}% | {y['net_units_sold']:,.0f} | {y['units_returned']:,.0f} | ${y['avg_ticket']:,.0f} | {y['total_transactions']:,} |"
    )
...
if len(data["yearly_totals"]) == 2:
    ...
    report.append("")
    report.append(
        f"**YoY Change 2024→2025:** Revenue: {rev_change:+.1f}% | Profit: {profit_change:+.1f}%"
    )
    report.append("")
...
sorted_cats = sorted(
    cat_data.keys(),
    key=lambda x: cat_data[x]
    .get(2024, cat_data[x].get(2025, {}))
    .get("net_revenue", 0),
    reverse=True,
)
...
for cat in sorted_cats[:20]:  # Top 20 categories
    for year in [2024, 2025]:
        if year in cat_data[cat]:
            c = cat_data[cat][year]
            margin = (
                (c["net_profit"] / c["net_revenue"] * 100) if c["net_revenue"] else 0
            )
            cat_display = cat[:30] if len(cat) > 30 else cat
            report.append(
                f"| {cat_display} | {year} | ${c['net_revenue']:,.0f} | ${c['net_profit']:,.0f} | {margin:.1f}% | ${c['avg_ticket']:,.0f} | {c['net_units']:,.0f} |"
            )
...
bestsellers_2024 = sorted(
    [b for b in data["bestsellers"] if b["year"] == 2024],
    key=lambda x: x["net_units_sold"],
    reverse=True,
)[:20]
for b in bestsellers_2024:
    ...
    report.append(
        f"| {cat} | {sub} | {b['sku']} | {prod} | {b['net_units_sold']:,.0f} | ${b['revenue']:,.0f} | ${b['profit']:,.0f} |"
    )
...
bestsellers_2025 = sorted(
    [b for b in data["bestsellers"] if b["year"] == 2025],
    key=lambda x: x["net_units_sold"],
    reverse=True,
)[:20]
...
for p in profitable_2024:
    ...
    report.append(
        f"| {cat} | {sub} | {p['sku']} | {prod} | ${p['total_profit']:,.0f} | {p['margin_percentage']:.1f}% | ${p['revenue']:,.0f} |"
    )
...
ticket_2024 = sorted(
    [
        t
        for t in data["avg_ticket_by_category"]
        if t["year"] == 2024 and t["avg_ticket"]
    ],
    key=lambda x: x["avg_ticket"],
    reverse=True,
)[:20]
for t in ticket_2024:
    ...
    report.append(
        f"| {cat} | {sub} | ${t['avg_ticket']:,.0f} | {t['num_transactions']:,} | ${t['net_revenue']:,.0f} | ${t['net_profit']:,.0f} |"
    )
...
ticket_2025 = sorted(
    [
        t
        for t in data["avg_ticket_by_category"]
        if t["year"] == 2025 and t["avg_ticket"]
    ],
    key=lambda x: x["avg_ticket"],
    reverse=True,
)[:20]
...
report.append(
    f"- **Return Rate:** {(total_returns/(total_units+total_returns)*100):.2f}%"
)
report.append(
    f"- **Categories:** {len(set(c['marca'] for c in data['category_totals']))}"
)
